...
let [frontendImage,setFrontendImage]=useState(null)//p5
let [backendImage,setBackendImage]=useState(null)//p5
let image=useRef()//p5
let {messages} = useSelector(state => state.message)

const handleImage=(e)=>{ //p5
  let file=e.target.files[0]
  setBackendImage(file)
  setFrontendImage(URL.createObjectURL(file))
    }
const handleSendMessage=async (e)=>{ //p5
  e.preventDefault()
...
{messages && messages.map((mess ,i)=>(
  mess.sender==userData._id?<SenderMessage image={mess.image} message={mess.message} key={i}/>:<ReceiverMessage image={mess.image} message={mess.message} key={i}/>
))}
    </div>
 {/* emoji box end */}
      </div>}
    ...
    <h1 className='text-gray-700 font-bold text-[50px]'>Welcome to QuickChat</h1>
