...
import { setOtherUsers, setUserData ,setSelectedUsers, setSearchData} from '../redux/userSlice';
...
// console.log(error)
  ...
  return (
    <div className={`${!selectedUser?"block":"hidden"} lg:w-[30%] w-full h-full overflow-hidden lg:block bg-slate-200  relative`}>
...
{/* search feature end */}
      ...
      <div className='w-full h-[300px] bg-[#20c7ff] rounded-b-[30%] shadow-gray-400 shadow-lg flex flex-col justify-center px-[20px] '>
           <h1 className='text-white font-bold text-[25px]'>QuickChat</h1>
...
{!search && otherUsers?.map((user ,i)=>(
  onlineUsers?.includes(user._id) && 
    <div key={i} className='relative rounded-full shadow-gray-500 bg-white shadow-lg flex justify-center items-center mt-[10px] cursor-pointer' onClick={()=>dispatch(setSelectedUsers(user))}>
    <div className='w-[60px] h-[60px]   rounded-full overflow-hidden flex justify-center items-center '>
    <img src={user.image || dp} alt="" className='h-[100%]'/>
    </div>
     <span className='w-[12px] h-[12px] rounded-full absolute bottom-[6px] right-[-1px] bg-[#3aff20] shadow-gray-500 shadow-md'></span>
...
{otherUsers?.map((user,i)=>(
    <div key={i} className='w-[95%] h-[60px] flex items-center gap-[20px] shadow-gray-500 bg-white shadow-lg rounded-full hover:bg-[#78cae5] cursor-pointer' onClick={()=>dispatch(setSelectedUsers(user))}>
